...
spark.sql(f"""
          CREATE TABLE IF NOT EXISTS Sat_Clientes_CorreosElectronicos 
          (
...
dfRegistrosNuevos = spark.sql(f"""
            ...
            SELECT 
                ...
                ,CASE 
                WHEN ISNULL(email) = TRUE THEN 'No Definido' 
                ELSE email
            ...
            LEFT JOIN Sat_Clientes_CorreosElectronicos AS B 
...
if not dfRegistrosNuevos.isEmpty():
    dfDeltaSatClientesTelefonos = DeltaTable.forName(spark, f"{catalogoDataVault}.{contenedorRegional}.Sat_Clientes_CorreosElectronicos")
